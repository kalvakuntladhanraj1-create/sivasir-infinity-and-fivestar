...
def generate_sale():
    try:
        ...
        if not os.path.exists(template_path):
            return f"SALE TEMPLATE NOT FOUND: {template_path}", 500
...
def generate_gift():
    try:
        ...
        if not os.path.exists(template_path):
            return f"GIFT TEMPLATE NOT FOUND: {template_path}", 500
